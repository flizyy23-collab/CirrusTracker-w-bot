...
class ReportChatEndpoint {
    async call(req, res) {
        const { token, reporter, username, message, displayName, rawMessage } = req.query;
        ...
        const packet = { data: { username, message, displayName: displayName || null, rawMessage: rawMessage || null } };
